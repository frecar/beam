...
describe('extractCodecFromAnnexB', () => {
  it('extracts Main profile codec string from 4-byte start code', () => {
    ...
    const data = new Uint8Array([0x00, 0x00, 0x00, 0x01, 0x67, 0x4d, 0x00, 0x33, 0xac, 0xd9]);
  ...
  it('extracts High profile codec string', () => {
    ...
    const data = new Uint8Array([0x00, 0x00, 0x00, 0x01, 0x67, 0x64, 0x00, 0x33, 0xac, 0xd9]);
  ...
  it('extracts codec from SPS after PPS in access unit', () => {
    ...
    const data = new Uint8Array([
      ...
      0x4d,
      0x40,
      0x28, // SPS: Main, constraint_set1, level 4.0
      0x00,
      0x00,
      0x00,
      0x01,
  ...
  it('extracts codec from 3-byte start code', () => {
    const data = new Uint8Array([0x00, 0x00, 0x01, 0x67, 0x4d, 0x00, 0x28, 0xac, 0xd9]);
    expect(extractCodecFromAnnexB(data)).toBe('avc1.4d0028');
  });

  it('returns null when no SPS present', () => {
    // Only IDR (type 5), no SPS
    const data = new Uint8Array([0x00, 0x00, 0x00, 0x01, 0x65, 0x88, 0x80, 0x40]);
  ...
  it('handles real nvh264enc Main profile SPS', () => {
    ...
    const data = new Uint8Array([
      0x00, 0x00, 0x00, 0x01, 0x67, 0x4d, 0x00, 0x28, 0xac, 0xd9, 0x40, 0x78, 0x02, 0x27, 0xe5,
      0xc0, 0x44, 0x00, 0x00, 0x03,
    ]);
    expect(extractCodecFromAnnexB(data)).toBe('avc1.4d0028');
  });
});
