...
### Fixed
- **Audio/video desync**: PulseAudio accumulated silence buffer during startup. Flush record stream before capture loop to discard stale data.
...
### Fixed
- **Slow session startup**: PulseAudio retry loop (5s) was blocking WebSocket connection. Moved to background thread so video streams immediately while audio connects asynchronously.
